...
{
public:
    using Ptr = std::shared_ptr<MessagePromise>;

    void doSuspend(std::function<void()> onSuspend)
    {
        m_exchangeMessagePromise = std::promise<CallParameters::UniquePtr>();
        onSuspend();
    }

    void awaitAndDo(std::function<void(CallParameters::UniquePtr)> onExchangeMessage)
    {
        onExchangeMessage(m_exchangeMessagePromise.get_future().get());
    }

    void resume(CallParameters::UniquePtr message)
    {
        m_exchangeMessagePromise.set_value(std::move(message));
    }

private:
    std::promise<CallParameters::UniquePtr> m_exchangeMessagePromise;
};

class MessagePromiseSwapper2
{
public:
    ...
    {
        auto lastPromise = m_currentMessagePromise;
        m_currentMessagePromise = std::make_shared<MessagePromise>();

        m_pool->enqueue(
            [lastPromise = m_currentMessagePromise, waitAndDo = std::move(waitAndDo)]() {
                lastPromise->awaitAndDo(
                    [waitAndDo = std::move(waitAndDo)](
                        CallParameters::UniquePtr message) { waitAndDo(std::move(message)); });
            });

        auto message = spawnCall();
        lastPromise->resume(std::move(message));

        m_currentMessagePromise->doSuspend([this, lastPromise, spawnCall = std::move(spawnCall)]() {
            auto message = spawnCall();
            ...
            {
                m_currentMessagePromise->resume(std::move(message));
            }
        });
    }

private:
    MessagePromise::Ptr m_currentMessagePromise;
...
public:
    ...
    PromiseTransactionExecutive(ThreadPool::Ptr pool, std::weak_ptr<BlockContext> blockContext,
