...
{
    Transactions txs;
    // Generate user + recever = 2 * _userNum
    for (size_t i = 0; i < 2 * _userNum; i++)
    {
        u256 value = 0;
        u256 gasPrice = 0;
        u256 gas = 10000000;
...
void genTxUserTransfer(Block& _block, size_t _userNum, size_t _txNum)
{
    ...
    for (size_t i = 0; i < _txNum; i++)
    {
        ...
        //*/
        //*
        string userFrom = to_string(i % _userNum);
        string userTo = to_string((_userNum + i) % (2 * _userNum));
        //*/
        u256 money = 1;
        dev::eth::ContractABI abi;
...
static void startExecute(int _totalUser, int _totalTxs)
{
    ...
    while (true)
    {
        ...
        /// parallel tbb concurrent queue execution
        {
            std::cout << "Generating transfer txs..." << std::endl;
            Block block;
            genTxUserTransfer(block, _totalUser, _totalTxs);
            std::cout << "parallel tbb concurrent queue executing txs..." << std::endl;
            blockVerifier->parallelTbbCqExecuteBlock(block, parentBlockInfo);
            std::cout << "Executed" << std::endl;
        }
